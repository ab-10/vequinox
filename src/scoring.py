...
def score_svg(svg_text, prompt, clip_processor, clip_model):
    """Get CLIP score for how well image matches prompt"""
    ...
    clip_inputs = clip_processor(text=[prompt], images=img, return_tensors="pt")
    clip_inputs = {k: v.to(clip_model.device) for k, v in clip_inputs.items()}
    with torch.no_grad():
        clip_outputs = clip_model(**clip_inputs)
    return img, clip_outputs.logits_per_image.item()
